...
def cut_i(b,i):
    ...
    for var in b:
        print(var)
        print(np.shape(b[var]))
        if isinstance(b[var],np.ndarray) and np.shape(b[var]) != ():
...
def read_case(filename):
    ...
    #setting up the lists for each variable
    g['ni'] = []
    g['nj'] = [] 
    ...
    for m in range(g['nn']):
        ...
        fieldnames = ['x','y','area','lx_i','ly_i','lx_j','ly_j']
        ni_mesh = np.array([0,0,-1,0,0,-1,-1]) + g['ni'][m]
        nj_mesh = np.array([0,0,-1,-1,-1,0,0]) + g['nj'][m]
        ...
        for n,name in enumerate(fieldnames):
            ...
            g[name][m] = np.reshape(g[name][m],[ni_mesh[n],nj_mesh[n]],order='F')

        # Always read the logical array describing the wall 
        
        value = np.fromfile(f,dtype=np.int32,count=ni*nj) == 1
        value = np.reshape(value,[ni,nj],order='F')
        g['wall'].append(value)
